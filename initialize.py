...
def initialize():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS flights (flight_id INT AUTO_INCREMENT PRIMARY KEY, scheduled_departure DATETIME, scheduled_arrival DATETIME, actual_departure DATETIME,
            actual_arrival DATETIME, passenger_count INT, projected_fuel_consumption FLOAT, actual_fuel_consumption FLOAT, distance FLOAT, aircraft_id VARCHAR(255), destination_airport_id VARCHAR(255),
            origin_airport_id VARCHAR(255), FOREIGN KEY (aircraft_id) REFERENCES aircrafts(registration_code), FOREIGN KEY (destination_airport_id) REFERENCES airports(icao_id), FOREIGN KEY (origin_airport_id) REFERENCES airports(icao_id))""")
